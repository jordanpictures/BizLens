...
exports.createBooking = async (req, res) => {
  const { customer_name, customer_phone, service_type, package, quantity, start_time, end_time, agreed_price, notes, amount_paid } = req.body;
  
  try {
    await db.query('BEGIN');
    
    // Convert array to string if multiple packages were selected
    const packageStr = Array.isArray(package) ? package.join(', ') : package;
    ...
    const result = await db.query(
      ...
      [customer_name || null, customer_phone || null, service_type, packageStr, quantity, start_time, end_time || null, agreed_price, notes]
    );
